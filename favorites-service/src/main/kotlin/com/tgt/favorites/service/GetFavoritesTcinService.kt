...
class GetFavoritesTcinService(
    ...
    fun getFavoritesTcin(guestId: String, tcins: String): Mono<List<FavoritesTcinResponseTO>> {
    ...
    private fun addItemDetailsToTcin(listGetAllResponseTO: ListGetAllResponseTO, tcinAndListItemDetailsMap: MutableMap<String, MutableList<ListItemDetailsTO>?>): Mono<Map<String, List<ListItemDetailsTO>?>> {
        ...
        for (i in listGetAllResponseTO.pendingItems!!.indices) {
            if (tcinAndListItemDetailsMap.containsKey(listGetAllResponseTO.pendingItems!![i].tcin)) {
                ...
                val x: MutableList<ListItemDetailsTO> = tcinAndListItemDetailsMap[listGetAllResponseTO.pendingItems!![i].tcin]!!
                x.add(ListItemDetailsTO(listGetAllResponseTO.listId, listGetAllResponseTO.pendingItems!![i].itemTitle, listGetAllResponseTO.pendingItems!![i].listItemId))
                tcinAndListItemDetailsMap[listGetAllResponseTO.pendingItems!![i].tcin!!] = x
            }
        }
    ...
    private fun formFavoritesTcinResponse(tcinAndListItemDetailsMap: Map<String, List<ListItemDetailsTO>?>): List<FavoritesTcinResponseTO> {

        val tcins = tcinAndListItemDetailsMap.keys.toList()
        val listOfListItemDetail = tcinAndListItemDetailsMap.values.toList()
        val listOfFavouritesTcinResponseTO = mutableListOf<FavoritesTcinResponseTO>()
        for (i in tcins.indices)
            listOfFavouritesTcinResponseTO.add(FavoritesTcinResponseTO(tcins[i], listOfListItemDetail[i]))
